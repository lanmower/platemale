      ...
      {docs.length ? <List>
        {
          docs.map((doc)=>{
            ...
            return (
              <ListDoc key={doc._id} _id={doc._id} info={info} doc={doc} history={history} match={match} collection={collection} config={config}></ListDoc>)
            })
        }
      </List>
        :
...
module.exports =  withStyles(styles)(ListComponent);
