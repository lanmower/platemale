...
var _handleRemove = _interopRequireDefault(require("./handleRemove"));
...
var viewRoute = function viewRoute(_ref) {
  ...
  var collection = config.offline ? collections.clientCollection : collections.serverCollection;
  return {
    path: "/" + collection._name + "/:_id",
    component: (0, _reactMeteorData.createContainer)(function (_ref2) {
...
var listRoute = function listRoute(_ref3) {
  ...
  return {
    ...
    component: (0, _reactMeteorData.createContainer)(function (_ref4) {
...
var submitRoute = function submitRoute(_ref5) {
  var collections = _ref5.collections,
      config = _ref5.config;
  var collection = collections.submissionsCollection;
  var navButtonStore = new ReactiveVar();
  return {
    path: "/" + collection._name,
    component: (0, _reactMeteorData.createContainer)(function (_ref6) {
...
var newRoute = function newRoute(_ref7) {
  var collections = _ref7.collections,
      config = _ref7.config;
  var collection = config.offline ? collections.submissionsCollection : collections.serverCollection;
  var navButtonStore = new ReactiveVar();
  return {
    path: "/" + collection._name + "/new",
    component: (0, _reactMeteorData.createContainer)(function (_ref8) {
...
var editRoute = function editRoute(_ref9) {
  var collections = _ref9.collections,
      config = _ref9.config;
  var collection = collections.serverCollection;
  var navButtonStore = new ReactiveVar();
  var title = "Edit " + (0, _capitalize.default)(config.name);
  return {
    path: "/" + collection._name + "/:_id/edit",
    component: (0, _reactMeteorData.createContainer)(function (_ref10) {
      var match = _ref10.match;
      var _id = match.params._id;
      var subscription = Meteor.subscribe(collection._name + '.view', _id);
      var doc = collection.findOne(_id);
      var loading = config.subscribe ? !subscription.ready() : false;
      var navButtons = navButtonStore.get();
      var title = "New " + (0, _splitWords.default)((0, _capitalize.default)(config.name));
      var before = config.before.edit;
      if (before) before();
      return {
        loading: loading,
        doc: doc,
...
var defaultRoutes = function defaultRoutes(collections) {
  ...
  if (offline) routes['submit'] = submitRoute({
    collections: collections,
    config: config
  });
  return routes;
